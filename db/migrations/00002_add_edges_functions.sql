...
CREATE FUNCTION extract_resource_details(resource_key text, part integer) RETURNS text
    LANGUAGE sql
AS
$$
...
CREATE FUNCTION generate_resource_name(resource_type integer, resource_name text, resource_variant text) RETURNS text
    LANGUAGE plpgsql
AS
$$
DECLARE
...
-- Function: process_json_array
create function process_json_array(array_data jsonb, from_type integer, from_name text, from_variant text, to_type integer) returns void
    language plpgsql
as
$$
DECLARE
    item JSONB;
BEGIN
    FOR item IN SELECT jsonb_array_elements(array_data)
        LOOP
            PERFORM insert_edge(
                    from_type,
                    from_name,
...
CREATE FUNCTION insert_edge(from_type integer, from_name text, from_variant text, to_type integer, to_name text, to_variant text) RETURNS void
    LANGUAGE plpgsql
AS
$$
BEGIN
...
create function process_feature_variant(feature_variant_key text, feature_variant_value text) returns void
    language plpgsql
as
$$
DECLARE
...
create function process_label_variant(label_variant_key text, label_variant_value text) returns void
    language plpgsql
as
$$
DECLARE
...
create function process_source_variant(source_variant_key text, source_variant_value text) returns void
    language plpgsql
as
$$
DECLARE
...
create function process_ts_variant(ts_key text, ts_value text) returns void
    language plpgsql
as
$$
DECLARE
...
DROP FUNCTION IF EXISTS process_json_array(jsonb, integer, text, text, integer);
