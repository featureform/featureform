...
the `--clickhouse` flag i.e.

```shell
featureform deploy docker --quickstart --clickhouse
```
...
We will be using Postgres for our example, however the ClickHouse provider works the same way,
and passing the `--clickhouse` flag during deployment should have downloaded a similar `definitions.py` file with
ClickHouse being the source provider.
