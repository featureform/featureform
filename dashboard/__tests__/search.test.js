...
describe("Search Input Tests", () => {
    ...
    test("Issue-762: A basic search string invokes the router.", async () => {
        ...
        expect(userRouterMock.push).toHaveBeenCalledWith(`${SEARCH_URI}${searchText}`);
    });
    ...
    test("Issue-762: The user can incrementally delete the search input", async () => {
        ...
        await userEvent.type(searchField, USER_EVENT_DELETE.repeat(searchText.length));
    ...
    `(`Issue-762: The search entry "$SearchInputParam" invokes the router: $InvokeParam times`, async ({ SearchInputParam, InvokeParam }) => {
        ...
        const searchField = helper.getByTestId(SEARCH_INPUT_ID);
        await userEvent.type(searchField, `${SearchInputParam}${USER_EVENT_ENTER}`);

        //then:
        expect(userRouterMock.push).toHaveBeenCalledTimes(InvokeParam);
        if (InvokeParam) {
            expect(userRouterMock.push).toHaveBeenCalledWith(`${SEARCH_URI}${SearchInputParam.trim()}`);
        }
    });
});
