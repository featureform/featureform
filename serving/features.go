...
func (iv indexedValue) Index() int {
	return iv.index
}

func (iv indexedValue) Value() interface{} {
	return iv.value
}
...
func (iv indexedFeatureRow) Index() int {
	return iv.index
}

func (iv indexedFeatureRow) Value() interface{} {
	return iv.values
}
...
func (serv *FeatureServer) getFeatureRows(ctx context.Context, features []*pb.FeatureID, entityMap map[string][]string) ([]*pb.ValueList, error) {
	...
	serv.fetchFeatures(ctx, features, entityMap, vals, errc)
...
func (serv *FeatureServer) fetchFeatures(ctx context.Context, features []*pb.FeatureID, entityMap map[string][]string, vals chan indexedFeatureRow, errc chan error) {
...
func (serv *FeatureServer) getPrecomputedValues(ctx context.Context, entityMap map[string][]string, meta *metadata.FeatureVariant) ([]indexedValue, error) {
	...
	if store, has := serv.Providers.Load(meta.Provider()); has {
		featureTable, err := serv.cacheFeatureTable(ctx, store.(provider.OnlineStore), meta.Name(), meta.Variant())
		if err != nil {
			return nil, err
		}

		featureValues, err := serv.getEntityValues(ctx, entity, featureTable)
		if err != nil {
			return nil, err
		}
...
func sortValues(values []indexedObj) []indexedObj {
