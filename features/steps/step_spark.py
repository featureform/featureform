...
def step_impl(context):
    context.file = context.spark.register_parquet_file(
        name=f"transaction_short_{context.name_suffix}",
        variant="e2e_testing",
        owner="featureformer",
...
def step_impl(context):
    ...
    def average_user_score():
        ...
        return f"SELECT CustomerID as user_id, avg(TransactionAmount) as avg_transaction_amt from {{{{ transaction_short_{context.name_suffix}.e2e_testing }}}} GROUP BY user_id"
    ...
    average_user_score.register_resources(
        ...
        features=[
            {"name": f"avg_transaction_{context.name_suffix}", "variant": "quickstart", "column": "avg_transaction_amt", "type": "float32"},
        ],
    )
...
def step_impl(context):
    context.file.register_resources(
        ...
        labels=[
            {"name": f"fraudulent_{context.name_suffix}", "variant": "quickstart", "column": "isfraud", "type": "bool"},
        ],
    )
...
def step_impl(context):
    ff.register_training_set(
        f"fraud_training_{context.name_suffix}", "quickstart",
        owner=f"featureformer_{context.name_suffix}",
        label=(f"fraudulent_{context.name_suffix}", "quickstart"),
        features=[(f"avg_transaction_{context.name_suffix}", "quickstart")],
    )
...
def step_impl(context):
    ...
    dataset = client.training_set(f"fraud_training_{context.name_suffix}", "quickstart")
