...
func Test_createStatFromResource(t *testing.T) {
	err := godotenv.Load(".env")
	if err != nil {
		fmt.Println(err)
	}
	...
	for name, provider := range map[string]Provider{"POSTGRES": pgProvider} {
		t.Run(name, func(t *testing.T) {
			...
			err = store.RegisterStatFromResourceTable(featureID, recSchema)
