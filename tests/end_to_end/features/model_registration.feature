Feature: Model Registration
  Scenario Outline: Registering a model with both non-existing and existing feature and training set
    ...
    And I register a feature on "<feature_column>" with type "<feature_type>" with "<entity_column>" and "<label_column>"
    ...
    Examples:
      | online_provider_type | offline_provider_type | dataset_path |  feature_column   | feature_type | entity_column | label_column |
      |        redis         |        postgres       | transactions | transactionamount |    float64   |   customerid  |   isfraud    |
