...
def test_no_models_registered_while_serving_training_set(provider_source_fxt,serving_client_fxt, is_local, is_insecure, request):
...
def test_registering_model_while_serving_training_set(provider_source_fxt,serving_client_fxt, is_local, is_insecure, request):
    custom_marks = [mark.name for mark in request.node.own_markers if mark.name != 'parametrize']
    provider, source, inference_store = request.getfixturevalue(provider_source_fxt)(custom_marks)
    serving_client = request.getfixturevalue(serving_client_fxt)(is_local, is_insecure)
...
def test_registering_two_models_while_serving_training_set(provider_source_fxt,serving_client_fxt, is_local, is_insecure, request):
    custom_marks = [mark.name for mark in request.node.own_markers if mark.name != 'parametrize']
    provider, source, inference_store = request.getfixturevalue(provider_source_fxt)(custom_marks)
    serving_client = request.getfixturevalue(serving_client_fxt)(is_local, is_insecure)
    ...
    ts_1 = serving_client.training_set("fraud_training", "quickstart", model=model_name_b)
    next(ts_1)
    ts_2 = serving_client.training_set("fraud_training", "quickstart", model=model_name_c)
    next(ts_2)
...
def test_registering_same_model_twice_while_serving_training_set(provider_source_fxt,serving_client_fxt, is_local, is_insecure, request):
    custom_marks = [mark.name for mark in request.node.own_markers if mark.name != 'parametrize']
    provider, source, inference_store = request.getfixturevalue(provider_source_fxt)(custom_marks)
    serving_client = request.getfixturevalue(serving_client_fxt)(is_local, is_insecure)
    ...
    ts_1 = serving_client.training_set("fraud_training", "quickstart", model=model_name_d)
    next(ts_1)
    ts_2 = serving_client.training_set("fraud_training", "quickstart", model=model_name_d)
    next(ts_2)
...
def test_registering_model_while_serving_features(provider_source_fxt,serving_client_fxt, is_local, is_insecure, request):
    custom_marks = [mark.name for mark in request.node.own_markers if mark.name != 'parametrize']
    provider, source, inference_store = request.getfixturevalue(provider_source_fxt)(custom_marks)
    serving_client = request.getfixturevalue(serving_client_fxt)(is_local, is_insecure)
...
def test_registering_two_models_while_serving_features(provider_source_fxt,serving_client_fxt, is_local, is_insecure, request):
    custom_marks = [mark.name for mark in request.node.own_markers if mark.name != 'parametrize']
    provider, source, inference_store = request.getfixturevalue(provider_source_fxt)(custom_marks)
    serving_client = request.getfixturevalue(serving_client_fxt)(is_local, is_insecure)
    ...
    fts_1 = serving_client.features([("avg_transactions", "quickstart")], {"user": "C1410926"}, model=model_name_f)
    fts_2 = serving_client.features([("avg_transactions", "quickstart")], {"user": "C1410926"}, model=model_name_g)
...
def test_registering_same_model_twice_while_serving_features(provider_source_fxt,serving_client_fxt, is_local, is_insecure, request):
    custom_marks = [mark.name for mark in request.node.own_markers if mark.name != 'parametrize']
    provider, source, inference_store = request.getfixturevalue(provider_source_fxt)(custom_marks)
    serving_client = request.getfixturevalue(serving_client_fxt)(is_local, is_insecure)
    ...
    fts_1 = serving_client.features([("avg_transactions", "quickstart")], {"user": "C1410926"}, model=model_name_h)
