...
var postgresConfigE2E provider.PostgresConfig

func TestCoordinatorCallsE2E(t *testing.T) {
	err := godotenv.Load(".env")
	postgresConfigE2E = provider.PostgresConfig{
		Host:     "localhost",
		Port:     "5432",
	...
	t.Run("Test Materialize", func(t *testing.T) {
		if err := testCoordinatorMaterializeFeatureE2E(t); err != nil {
...
func materializeFeatureWithProviderE2E(client *metadata.Client, offlineConfig provider.SerializedConfig, onlineConfig provider.SerializedConfig, featureName string, sourceName string, originalTableName string) error {
	...
	if err := client.CreateAll(context.Background(), defs); err != nil {
		fmt.Println("Could not create all", err)
		return err
	}
	return nil
}
...
func testCoordinatorMaterializeFeatureE2E(t *testing.T) error {
	...
	//_, err = clientv3.New(clientv3.Config{Endpoints: []string{"localhost:2379"}})
	//if err != nil {
	//	return err
	//}
	////defer cli.Close()
	fmt.Printf("%#v\n", postgresConfigE2E)
	serialPGConfigE2E := postgresConfigE2E.Serialize()
	fmt.Printf("%#v\n", serialPGConfigE2E)
	offlineProvider, err := provider.Get(provider.PostgresOffline, serialPGConfigE2E)
	if err != nil {
		t.Fatalf("could not get offline provider: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("could not get online provider: %v", err)
	}
	onlineStore, err := p.AsOnlineStore()
	if err != nil {
		t.Fatalf("could not get provider as online store")
	}
	onlineStore.CreateTable("test", "test", provider.String)
	schemaInt := provider.TableSchema{
		Columns: []provider.TableColumn{
			{Name: "entity", ValueType: provider.String},
			{Name: "value", ValueType: provider.Int},
	...
	for _, value := range testOfflineTableValues {
		if err := featureTable.Write(value); err != nil {
			t.Fatalf("could not write to offline feature table")
		}
	}
	fmt.Println("HERE")
	originalTableName := uuid.New().String()
	if err := CreateOriginalPostgresTableE2E(originalTableName); err != nil {
		t.Fatalf("%v", err)
	}
	fmt.Println("HERE")
	if err := materializeFeatureWithProviderE2E(client, serialPGConfigE2E, serialRedisConfig, featureName, sourceName, originalTableName); err != nil {
	...
	if err != nil {
		t.Fatalf("could not get feature: %v", err)
	}
	if featureCreated.Status() != metadata.CREATED {
		t.Fatalf("Feature not set to created with no coordinator running")
	}

	featureComplete, err := client.GetFeatureVariant(context.Background(), metadata.NameVariant{Name: featureName, Variant: ""})
	if err != nil {
		t.Fatalf("could not get feature variant")
	}
	fmt.Printf("%#v\n", featureComplete.Name())
	...
	for _, record := range testOfflineTableValues {
		...
		if !reflect.DeepEqual(value, record.Value) {
			t.Fatalf("Feature value did not materialize")
		}
	}
	return nil
}
...
func CreateOriginalPostgresTableE2E(tableName string) error {
	url := fmt.Sprintf("postgres://%s:%s@%s:%s/%s", postgresConfigE2E.Username, postgresConfigE2E.Password, postgresConfigE2E.Host, postgresConfigE2E.Port, postgresConfigE2E.Database)
	ctx := context.Background()
