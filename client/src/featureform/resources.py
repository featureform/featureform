...
class TrainingSet:
    ...
    def _check_insert_training_set_resources(self, db) -> None:
        try:
            db.get_name_variant("labels_variant", "label_name", self.label[0], "variant_name", self.label[1])
        except ValueError:
            raise ValueError("{} does not exist. Failed to register training set".format(self.label[0]))
        for feature in self.features:
            try:
                db.get_name_variant("feature_variant", "feature_name", feature[0], "variant_name", feature[1])
            except ValueError:
