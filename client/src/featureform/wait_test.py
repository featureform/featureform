...
def test_not_ready_timeout_training_set(mocker):
    ...
    try:
        training_set = dataset = client.training_set(TRAININGSET_NAME, TRAININGSET_VARIANT).wait(timeout=timedelta(seconds=1))
...
def test_timeout_feature(mocker):
    ...
    try:
        ...
        features = client.features([(FEATURE_NAME, FEATURE_VARIANT)], {FEATURE_ENTITY: FEATURE_VALUE}).wait(timeout=timedelta(seconds=1))
    except ValueError as actual_error:
        assert actual_error.message == "Waited too long for resource to be ready"
...
def test_ready_feature(mocker):
    ...
    actual = client.features([(FEATURE_NAME, FEATURE_VARIANT)], {FEATURE_ENTITY: FEATURE_VALUE}).wait(timeout=timedelta(seconds=1))
    assert expected == actual
