...
-- Helper functions to parse and create resource keys
...
-- Check if the edges table is empty and populate it if necessary
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM edges LIMIT 1) THEN
        PERFORM add_edges_from_all_ff_task_metadata();
ELSE
        RAISE NOTICE 'Skipping call to add_edges_from_all_ff_task_metadata because the edges table is not empty.';
END IF;
END;
$$;
