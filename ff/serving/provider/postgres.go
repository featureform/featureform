...
// Plan of action: Need to try to make a standard table that can hold all kinds
// of data or make different tables customised for each type of visualization
// It should get converted to json object in dashboard_metadata unless its really
// helpful here
// Figure out how the database works, need to make my own schema. Aka do i need a
// different schema for each type of visualisation?
// Compute the stats in a different file and then send it here and put it into the table
// Have a function here that takes the data and stores it in the database table
// Figure out what database are we querying
package provider
...
func (store *postgresOfflineStore) RegisterResourceFromSourceTable(id ResourceID, schema ResourceSchema) (OfflineTable, error) {
	...
	//_, err := store.RegisterStatFromResourceTable(id, schema)
	//if err != nil {
	//	fmt.Println("Could not register from Primary Table: %s", err)
	//}
...
func (store *postgresOfflineStore) RegisterStatFromResourceTable(id ResourceID, schema ResourceSchema) error {

	if schema.Entity == "" || schema.Value == "" {
		return fmt.Errorf("non-empty entity and value columns required")
	}
	tableName := store.getResourceTableName(id)
	store.chooseMetric(tableName)
	return err
}
...
func newPostgresTrainingSetIterator(rows pgx.Rows, columnTypes []postgresColumnType) TrainingSetIterator {
...
func newPostgresFeatureIterator(rows pgx.Rows, columnType postgresColumnType) FeatureIterator {
...
func newPostgresGenericTableIterator(rows pgx.Rows, columnTypes []postgresColumnType, columnNames []string) GenericTableIterator {
