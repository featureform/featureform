...
def test_user():
    req = requests.get("http://localhost:8000/data/users", json=True)
    json_ret = req.json()
    assert (json_ret == users)


def test_providers():
    req = requests.get("http://localhost:8000/data/providers", json=True)
    json_ret = req.json()
...
def test_entities():
    req = requests.get("http://localhost:8000/data/entities", json=True)
    json_ret = req.json()
    assert (json_ret == entities)

def test_features():
    req = requests.get("http://localhost:8000/data/features", json=True)
    json_ret = req.json()
    for res in json_ret:
...
def test_labels():
    req = requests.get("http://localhost:8000/data/labels", json=True)
    json_ret = req.json()
    for res in json_ret:
        for v in res['variants']:
            del res['variants'][v]['created']
...
def test_training_sets():
    req = requests.get("http://localhost:8000/data/training-sets", json=True)
    json_ret = req.json()
    for res in json_ret:
        for v in res['variants']:
            del res['variants'][v]['created']
